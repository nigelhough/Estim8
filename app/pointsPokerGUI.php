...
class pointsPokerGUI
{
    ...
    public function __construct() {
        $this->storyEstimation = new \estim8\storyEstimation\storyEstimation();
        $this->pointsPokerGUI();
    }
    ...
    private function pointsPokerGUI() {
        ...
        switch($status) {
            ...
            case \estim8\storyEstimation\storyEstimationState::VOTING:
                // Story added, show the voting buttons
                $this->showStory();
                $this->showVotingOptions();
                $this->showButtons();
                break;
            case \estim8\storyEstimation\storyEstimationState::ESTIMATE:
                ...
                $this->showButtons();
                break;
            case \estim8\storyEstimation\storyEstimationState::RESULT:
    ...
    private function showButtons() {
        ...
        if($this->storyEstimation->getState() === \estim8\storyEstimation\storyEstimationState::VOTING) {
            if($this->storyEstimation->getVotesCount()) {
                $html .= "<a href='?end_voting=1' class='poker-button btn btn-success'>Finish Voting</a>";
            }
        } else if($this->storyEstimation->getState() === \estim8\storyEstimation\storyEstimationState::ESTIMATE) {
            if($this->storyEstimation->getVotesCount()) {
    ...
    private function showVotingOptions() {
        ...
        $param = 'vote';
        if($this->storyEstimation->getState() === \estim8\storyEstimation\storyEstimationState::ESTIMATE) {
